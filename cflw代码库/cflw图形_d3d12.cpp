...
C着色器工厂2 &C三维::fg着色器工厂() {
	if (m着色器工厂 == nullptr) {
		m着色器工厂 = std::make_unique<C着色器工厂2>();
		m着色器工厂->f初始化();
	}
	return *m着色器工厂;
}
...
//编译着色器
HRESULT C着色器工厂2::f编译顶点着色器(ComPtr<IDxcBlob> &a, const wchar_t *a文件名, const wchar_t *a函数名) {
	return f编译着色器(a, a文件名, a函数名, L"vs_6_0");
}
HRESULT C着色器工厂2::f编译像素着色器(ComPtr<IDxcBlob> &a, const wchar_t *a文件名, const wchar_t *a函数名) {
	return f编译着色器(a, a文件名, a函数名, L"ps_6_0");
}
HRESULT C着色器工厂2::f编译几何着色器(ComPtr<IDxcBlob> &a, const wchar_t *a文件名, const wchar_t *a函数名) {
	return f编译着色器(a, a文件名, a函数名, L"gs_6_0");
}
HRESULT C着色器工厂2::f编译外壳着色器(ComPtr<IDxcBlob> &a, const wchar_t *a文件名, const wchar_t *a函数名) {
	return f编译着色器(a, a文件名, a函数名, L"hs_6_0");
}
HRESULT C着色器工厂2::f编译域着色器(ComPtr<IDxcBlob> &a, const wchar_t *a文件名, const wchar_t *a函数名) {
	return f编译着色器(a, a文件名, a函数名, L"ds_6_0");
}
HRESULT C着色器工厂2::f编译计算着色器(ComPtr<IDxcBlob> &a, const wchar_t *a文件名, const wchar_t *a函数名) {
	return f编译着色器(a, a文件名, a函数名, L"cs_6_0");
}
//核心函数
HRESULT C着色器工厂2::f编译着色器(ComPtr<IDxcBlob> &a输出, const wchar_t *a文件名, const wchar_t *a入口, const wchar_t *a着色模型) {
	HRESULT hr;
...
HRESULT C着色器工厂2::f读取着色器(ComPtr<IDxcBlob> &a, const wchar_t *a文件名) {
	HRESULT hr;
	ComPtr<IDxcBlobEncoding> v源;
	hr = m库->CreateBlobFromFile(a文件名, nullptr, &v源);
	if (FAILED(hr)) {
...
S着色器字节代码 S着色器字节代码::fc二进制大对象(IDxcBlob *a) {
	return {a->GetBufferPointer(), a->GetBufferSize()};
}
...
void S图形管线参数::fs顶点着色器(IDxcBlob *a) {
	VS = S着色器字节代码::fc二进制大对象(a);
}
void S图形管线参数::fs像素着色器(IDxcBlob *a) {
	PS = S着色器字节代码::fc二进制大对象(a);
}
void S图形管线参数::fs几何着色器(IDxcBlob *a) {
	GS = S着色器字节代码::fc二进制大对象(a);
}
void S图形管线参数::fs外壳着色器(IDxcBlob *a) {
	HS = S着色器字节代码::fc二进制大对象(a);
}
void S图形管线参数::fs域着色器(IDxcBlob *a) {
	DS = S着色器字节代码::fc二进制大对象(a);
}
