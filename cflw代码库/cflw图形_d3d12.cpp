...
C渲染状态::C渲染状态() {
	...
	m采样器.m纹理.AddressU = D3D12_TEXTURE_ADDRESS_MODE_MIRROR;
	m采样器.m纹理.AddressV = D3D12_TEXTURE_ADDRESS_MODE_MIRROR;
	m采样器.m纹理.AddressW = D3D12_TEXTURE_ADDRESS_MODE_MIRROR;
...
D3D12_SAMPLER_DESC C渲染状态::f采样器转换(const D3D12_STATIC_SAMPLER_DESC &a) {
	return {a.Filter, a.AddressU, a.AddressV, a.AddressW, a.MipLODBias, a.MaxAnisotropy, a.ComparisonFunc, {0, 0, 0, 0}, a.MinLOD, a.MaxLOD};
}
D3D12_STATIC_SAMPLER_DESC C渲染状态::f采样器转换(const D3D12_SAMPLER_DESC &a, UINT a寄存器, UINT a空间, D3D12_SHADER_VISIBILITY a可视性) {
...
C根签名参数 &C根签名参数::f添加采样器(const D3D12_SAMPLER_DESC &a采样器, UINT a寄存器, UINT a空间, E着色器 a可见性) {
	ma采样器.push_back(C渲染状态::f采样器转换(a采样器, a寄存器, a空间, f计算可见性(a可见性)));
	return *this;
}
C根签名参数 &C根签名参数::f添加范围(E类型 a类型, UINT a数量, UINT a开始寄存器, UINT a寄存器空间, UINT a偏移, E着色器 a可见性) {
	assert(mw连续添加 == false);	//连续添加范围时不能添加单个范围
	assert(a数量 > 0);
...
void C根签名参数::fs标记(E根签名标记 a) {
	fs标记((D3D12_ROOT_SIGNATURE_FLAGS)a);
}
void C根签名参数::fs标记(D3D12_ROOT_SIGNATURE_FLAGS a) {
...
S资源栅栏 S资源栅栏::fc变换(ID3D12Resource* a资源, D3D12_RESOURCE_STATES a前, D3D12_RESOURCE_STATES a后, UINT a子资源, D3D12_RESOURCE_BARRIER_FLAGS a标记) {
	S资源栅栏 v栅栏{};
