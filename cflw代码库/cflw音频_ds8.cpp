...
HRESULT C音频引擎::f初始化 (HWND p窗口) {
	...
	if(FAILED(hr))
		return hr;
	hr = v设备->SetCooperativeLevel(p窗口, DSSCL_PRIORITY);
	DSBUFFERDESC v缓存描述;
...
//载入音乐
HRESULT C音频引擎::f载入音乐 (S声音 &p声音, WCHAR *p文件名, WCHAR *p文件名2) {
	if (p声音.v缓存)
		p声音.f释放();
	C波形文件 v文件[2];
	v文件[0].f打开(p文件名);
...
//载入音效
HRESULT C音频引擎::f载入音效 (S声音 &p声音, WCHAR *p文件名) {
	if (p声音.v缓存)
		p声音.f释放();
	C波形文件 v文件;
	HRESULT hr;
...
HRESULT C音频引擎::f注册缓存 (S声音 &p声音, C波形文件 &p文件) {
	DSBUFFERDESC v缓存描述;
	v缓存描述.dwFlags = DSBCAPS_STATIC;
	v缓存描述.dwBufferBytes = p文件.v块信息.cksize;
	v缓存描述.lpwfxFormat = p文件.v格式;
	LPDIRECTSOUNDBUFFER v临时 = nullptr;
	HRESULT hr;
	hr = v设备->CreateSoundBuffer(&v缓存描述, &v临时, nullptr);
	if (FAILED(hr))
		return hr;
	v临时->QueryInterface(IID_IDirectSoundBuffer8, (void**)&p声音.v缓存);
	if (FAILED(hr))
		return hr;
	v临时->Release();
	p声音.v大小 = v缓存描述.dwBufferBytes;
	return S_OK;
}
HRESULT C音频引擎::f填充声音 (S声音 &p声音, C波形文件 &p文件) {
	HRESULT hr;
	BYTE* v波形数据;
	UINT v波形大小;
	void* v数据[2] = {nullptr, nullptr};
	ULONG v长度[2];
	const UINT v文件大小 = p文件.v块信息.cksize;
	v波形数据 = new BYTE[v文件大小];
	if (v波形数据 == nullptr) {
		delete[] v波形数据;
		return S_FALSE;	//内存不足
	}
	hr = p文件.f读取(v文件大小, v波形数据, &v波形大小);
	if (FAILED(hr)) {
		delete[] v波形数据;
		return hr;	//读取失败
	}
	p文件.f重置();
	hr = p声音.v缓存->Lock(0, p声音.v大小, &v数据[0], &v长度[0], &v数据[1], &v长度[1], 0);
	if (FAILED(hr)) {
		delete[] v波形数据;
		return hr;	//锁定失败
	}
	memcpy(v数据[0], v波形数据, p声音.v大小);
	p声音.v缓存->Unlock(v数据[0], p声音.v大小, nullptr, 0);
	delete[] v波形数据;
	return S_OK;
}
//==============================================================================
...
HRESULT C波形文件::f读取(UINT p要读取大小, BYTE* a数据, UINT* p读取大小) {
	return WaveReadFile(m多媒体io, p要读取大小, a数据, &m块信息, p读取大小);
}
HRESULT C波形文件::f关闭() {
	mmioClose(m多媒体io, 0);
	return S_OK;
