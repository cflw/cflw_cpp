...
S二次函数 &S二次函数::fs(const S向量2 &p1, const S向量2 &p2, const S向量2 &p3) {
...
S直线方程 S直线方程::f平移(const S向量2 &p) const {
	S直线方程 v = *this;
	v.c -= v.a * p.x;
	v.c -= v.b * p.y;
	return v;
...
bool S直线方程::f平行(const S直线方程 &p) const {
	if (a == 0) return p.a == 0;
	else return (b / a) == (p.b / p.a);
}
...
S向量2 S直线方程::f交点(const S直线方程 &p) const {
	const float y = (p.a*c - a * p.c) / (p.b*a - b * p.a);
	const float x = f求x(y);
	return S向量2(x, y);
}
...
bool S直线方程3::f相交判定(const S直线方程3 &p, S向量3 *a输出) const {
	const float v[2] = {fg模(), p.fg模()};
	auto f = [](float *p1, float *p2)->float {
		return (p2[1] - p1[1]) / (p1[0] - p2[2]);
	};
	if (x[0] / v[0] == p.x[0] / v[1]) {
		return true;
	} else if (y[0] / v[0] == p.y[0] / v[1]) {
		return true;
	} else if (z[0] / v[0] == p.z[0] / v[1]) {
		return true;
	}
	return false;
}
...
S指数函数 &S指数函数::fs原点(const S向量2 &o, const S向量2 &p1, const S向量2 &p2) {
	...
	const float m符号 = ((p1.y - d > 0) && (p2.y - d > 0)) ? 1.f : -1.f;
	b = pow(v1, v2);
	a = (p1.y - d) / pow(b, p1.x + c) * m符号;
