...
namespace cflw::辅助 {
//删除指针
template<typename t> inline void f删除(t *&a) {
	if (a) {
		delete a;
		a = nullptr;
	}
...
//删除数组
template<typename t> inline void f删除数组(t *&a) {
	if(a) {
		delete[] a;
		a = nullptr;
	}
...
//强制删除
template<typename t> inline void f强制删除(t *&a) {
	delete a;
	a = nullptr;
}
//取数组元素数量
...
//内存清空
template<typename t> inline void f清零(t &a) {
