...
// 函数
//=============================================================================
template<> S颜色 f插值<S颜色>(const S颜色 &p起始值, const S颜色 &p终止值, float p中间值) {
	return p起始值.f全插值(p终止值, p中间值);
}
template<> S四元数 f插值(const S四元数 &q0, const S四元数 &q1, float p) {
	return q0.f插值(q1, p);
}
//=============================================================================
// 颜色rgb
...
// 世界矩阵2
//=============================================================================
S世界矩阵2::S世界矩阵2(const S向量2 &p平移, const float &p旋转, const S向量2 &p缩放): m平移(p平移), m旋转(p旋转), m缩放(p缩放) {
}
...
// 世界矩阵3
//=============================================================================
S世界矩阵3::S世界矩阵3(const S向量3 &p平移, const S向量3 &p旋转, const S向量3 &p缩放): m平移(p平移), m旋转(p旋转), m缩放(p缩放) {
}
...
// 相机
//=============================================================================
S相机::S相机(const S向量3 &p坐标, const S向量3 &p目标, const S向量3 &p上方): m坐标(p坐标), m目标(p目标), m上方(p上方) {
}
