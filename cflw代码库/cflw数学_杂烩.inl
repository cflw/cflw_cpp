...
// 位置计算
//==============================================================================
template<typename t> t C位置计算<t>::f左到中(const t &p位置, const t &p长度) {
	return p位置 + p长度 / 2;
}
template<typename t> t C位置计算<t>::f中到左(const t &p位置, const t &p长度) {
	return p位置 - p长度 / 2;
}
template<typename t> t C位置计算<t>::f右到中(const t &p位置, const t &p长度) {
	return p位置 - p长度 / 2;
}
template<typename t> t C位置计算<t>::f中到右(const t &p位置, const t &p长度) {
	return p位置 + p长度 / 2;
}
template<typename t> t C位置计算<t>::f右到左(const t &p位置, const t &p长度) {
	return p位置 - p长度;
}
template<typename t> t C位置计算<t>::f左到右(const t &p位置, const t &p长度) {
	return p位置 + p长度;
}
template<typename t> t C位置计算<t>::f下到中(const t &p位置, const t &p长度) {
	return p位置 + p长度 / 2;
}
template<typename t> t C位置计算<t>::f下到上(const t &p位置, const t &p长度) {
	return p位置 + p长度;
}
template<typename t> t C位置计算<t>::f中到下(const t &p位置, const t &p长度) {
	return p位置 - p长度 / 2;
}
template<typename t> t C位置计算<t>::f中到上(const t &p位置, const t &p长度) {
	return p位置 + p长度 / 2;
}
template<typename t> t C位置计算<t>::f上到中(const t &p位置, const t &p长度) {
	return p位置 - p长度 / 2;
}
template<typename t> t C位置计算<t>::f上到下(const t &p位置, const t &p长度) {
	return p位置 - p长度;
}
//==============================================================================
// 有限值
