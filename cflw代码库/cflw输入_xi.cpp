...
void f处理方向向量死区(C方向3 &a方向, SHORT x, SHORT y, float a死区 = c死区) {
	...
	if (m计时器.f滴答()) {
		...
		if (dr == ERROR_SUCCESS) {
			if (m包数 != v状态.dwPacketNumber) {
				...
				//摇杆
				f处理方向向量死区(m摇杆[(int)E摇杆::e左], v状态.Gamepad.sThumbLX, v状态.Gamepad.sThumbLY);
				f处理方向向量死区(m摇杆[(int)E摇杆::e右], v状态.Gamepad.sThumbRX, v状态.Gamepad.sThumbRY);
				//触发
