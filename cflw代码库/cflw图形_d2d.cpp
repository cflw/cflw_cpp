...
DWRITE_TEXT_RANGE F文本范围计算::operator()(int p左, int p右) const {
	const int v文本长度 = (int)m文本长度;
	assert(p左 >= -v文本长度);
	assert(p左 < v文本长度);
	assert(p右 > -v文本长度);
	assert(p右 <= v文本长度);
	const int v左 = (p左 < 0) ? (v文本长度 + p左) : p左;
	const int v右 = (p右 <= 0) ? (v文本长度 + p右) : p右;
	const int v长度 = v右 - v左;
	assert(v长度 > 0);
...
HRESULT C文本效果::DrawUnderline(void* a上下文, FLOAT x, FLOAT y, DWRITE_UNDERLINE const* p下划线, IUnknown* a效果) {
	HRESULT hr;
	D2D1_RECT_F v矩形{0, p下划线->offset, p下划线->width, p下划线->offset + p下划线->thickness};
...
HRESULT C文本效果::DrawInlineObject(void* a上下文, FLOAT x, FLOAT y, IDWriteInlineObject* inlineObject, BOOL aw旁边, BOOL aw反向, IUnknown* a效果) {
	return E_NOTIMPL;
}
...
//
//HRESULT C文本效果::f画文本(IDWriteTextLayout *a布局) {
//	return a布局->Draw(nullptr, this, m坐标计算->x(m坐标.x), m坐标计算->y(m坐标.y));
//}
//HRESULT C文本效果::f画文本(const std::wstring &a文本) {
//	assert(m文本格式);
//	HRESULT hr;
//	IDWriteTextLayout *m布局 = nullptr;
//	hr = m写字工厂->CreateTextLayout(a文本.c_str(), a文本.size(), m文本格式.Get(), 0, 0, &m布局);
//	if (FAILED(hr)) {
//		return hr;
//	}
//	hr = f画文本(m布局);
