...
HRESULT C音频::f创建声音(tp声音 &a声音, const wchar_t *a文件) {
	HRESULT hr;
	C波形文件 v文件;
	hr = v文件.f打开(a文件);
	if (FAILED(hr)) {
...
// 波形文件
//==============================================================================
HRESULT C波形文件::f打开(const wchar_t *a文件名) {
	m文件 = CreateFileW(a文件名, GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, 0, nullptr);
